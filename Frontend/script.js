...
function renderTickets(ticketsToRender) {
  ...
  ticketsToRender.forEach(ticket => {
    ...
    article.setAttribute("data-status", ticket.status);
    article.setAttribute("data-priority", ticket.priorityLevel);
...
function getTicketCountsByStatus(ticketsToCount) {
  return ticketsToCount.reduce((acc, ticket) => {
...
function sortTicketsByPriorityThenDate(ticketsToSort) {
  return [...ticketsToSort].sort((a, b) => {
    const priorityCompare = PRIORITY_RANK[a.priorityLevel] - PRIORITY_RANK[b.priorityLevel];
...
function getAverageResolutionDays(ticketsToAverage) {
  const closedTickets = ticketsToAverage.filter(t => t.closedDate);
...
function getTicketsByAssignee(ticketsToFilter, assignee) {
  if (!assignee || assignee === "") {
    return ticketsToFilter.filter(t => !t.assignedTo || t.assignedTo === "");
  }
  return ticketsToFilter.filter(t => t.assignedTo && t.assignedTo.toLowerCase() === assignee.toLowerCase());
}
...
function searchTickets(ticketsToSearch, keyword) {
  ...
  return ticketsToSearch.filter(ticket => 
    ...
    ticket.tags.some(tag => tag.toLowerCase().includes(lowerKeyword)) ||
    ticket.comments.some(comment => comment.text.toLowerCase().includes(lowerKeyword))
  );
}
...
function getTicketsSortedByDate(ticketsToSort) {
  const sortedCopy = [...ticketsToSort];
  sortedCopy.sort((a, b) => new Date(b.createdDate) - new Date(a.createdDate));
  return sortedCopy;
}
