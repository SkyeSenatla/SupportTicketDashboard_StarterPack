...
function getAverageResolutionDays(ticketsToAverage) {
  ...
  const totalDays = closedTickets.reduce((total, ticket) => {
    ...
    const days = (closed - created) / millisecondsPerDay;
...
function getTicketsByAssignee(ticketsToFilter, assignee) {
  if (assignee === null || assignee.trim() === "") {
    return ticketsToFilter.filter(ticket => ticket.assignedTo === null);
  }
  ...
  return ticketsToFilter.filter(ticket =>
    ...
    ticket.assignedTo.toLowerCase() === normalizedAssignee
  );
}
...
function searchTickets(ticketsToSearch, keyword) {
  ...
  return ticketsToSearch.filter(ticket => {
    ...
    return titleMatches || tagMatches || commentMatches;
  });
}
...
getTicketsByAssignee(tickets, "Maya Patel").forEach(t =>
  console.log(`#${t.id} ${t.title}`)
);

console.log("=== Unassigned Tickets ===");
getTicketsByAssignee(tickets, null).forEach(t =>
  console.log(`#${t.id} ${t.title}`)
);

console.log('=== Search Results For "network" ===');
searchTickets(tickets, "network").forEach(t =>
  console.log(`#${t.id} ${t.title}`)
);
...
getTicketsSortedByDate(tickets).forEach(t =>
